...
- **Bản chất:** Mâu thuẫn "ON rẻ vs spot căng" **không** phải nghịch lý — VND **dư tức thời** và **chênh lãi VND–USD ngắn hạn thu hẹp mạnh (ON chỉ +0,55%)** nên carry không đủ bù **kỳ vọng mất giá** → không ai muốn giữ VND ngắn hạn → ép spot. Vấn đề là **giá vốn/carry VND, không phải thiếu VND**.
...
### 2.1 Swap points theo tenor (ra ĐỒNG) — ĐÃ SỬA
> **Đính chính (lỗi bản trước):** đường cong niêm yết CHÍNH LÀ chênh lãi `(i_VND − i_USD)` annualized (dương = VND cao hơn USD). Swap points = `Spot × (giá trị bảng)/100 × days/360` — **lấy thẳng giá trị bảng, KHÔNG trừ thêm i_USD**. Bản trước coi bảng là i_VND tuyệt đối rồi trừ i_USD lần nữa → sai DẤU và độ lớn.

| Tenor | (i_VND−i_USD) mid % | days | Swap points (đồng) | Forward USD/VND |
|---|---|---|---|---|
| ON | 0.55 | 1 | **+0,4** | 26.430 |
| 1W | 3.70 | 7 | **+19,0** | 26.449 |
| 2W | 3.30 | 14 | **+33,9** | 26.464 |
| 3W | 3.55 | 21 | **+54,7** | 26.485 |
| 1M | 3.35 | 30 | **+73,8** | 26.504 |
| 2M | 3.45 | 60 | **+152** | 26.582 |
| 3M | 3.35 | 90 | **+221** | 26.651 |
| 6M | 3.65 | 180 | **+482** | 26.912 |
| 9M | 3.70 | 270 | **+734** | 27.164 |
| 1Y | 3.80 | 360 | **+1.004** | 27.434 |

- **Tất cả swap points DƯƠNG → USD forward premium** (forward USD/VND > spot, VND mất giá kỳ hạn) — đúng bản chất VND lãi cao hơn USD. (KHÔNG phải "forward discount/âm" như bản trước.)
- **1W là ĐỈNH chênh lãi short-end (3.70%)**, KHÔNG phải "lõm về 0": carry/ngày ở 1W cao nhất đoạn ngắn = turn premium ôm đêm chốt quý.
- Swap points tăng dần theo kỳ hạn (1Y ~+1.004 đồng ≈ 3.8% spot) thuần do **carry × thời gian**, khớp mức forward USD/VND thực tế.
...
- **Áp lực tỷ giá KHÔNG do macro xấu** mà do **chênh lãi VND–USD ngắn hạn mỏng (không đủ bù mất giá kỳ vọng) + kỳ vọng mất giá + mùa vụ + tâm lý**.
...
- **Fed & USD rate:** SOFR 3.62% ⇒ band suy ra ~3.50–3.75% [ƯỚC LƯỢNG]; Fed **gần trung tính**, kỳ vọng **giữ hoặc 1–2 lần cắt 25bp nửa cuối 2026**. Chân USD term ~**3.4–3.6%** (3M–1Y) — chỉ cần để **tách ra i_VND tuyệt đối**, KHÔNG cần để tính swap points (bảng đã là chênh lãi).
...
- **Hàm ý:** Fed nghiêng cắt (i_USD↓) → chênh lãi VND–USD **nới rộng** → swap points dâng. DXY hạ + CNY ổn → **giảm sức ép VND**. Ngược lại "higher for longer" / DXY>103 / CNH>7.30 → **cộng hưởng ép VND**.
...
- **DB-4 — chênh lãi đuôi:** 3M–1Y giữ **dương ~3.3–3.9%**; swap points 1Y trong **~+950 đến +1.080 điểm**. *Xác suất 55%.* *Pre-mortem:* Fed cắt sớm (USD↓) làm chênh lãi nới rộng thêm, hoặc SBV nâng nền VND.
...
5. **i_USD ước lượng** KHÔNG ảnh hưởng swap points (bảng đã là chênh lãi) — chỉ ảnh hưởng khi muốn tách i_VND tuyệt đối.
...
| Chân USD term (1W–1Y) | [ƯỚC LƯỢNG] — nhưng **swap points KHÔNG phụ thuộc** (đường cong đã là chênh lãi); chỉ cần i_USD nếu muốn TÁCH ra i_VND tuyệt đối |
